...
/* We are going to create a nexmo object from the node sdk and call it "nexmo",
...
nexmo.applications.update(app_id, "DEMO_APP", "voice", server_url + "/answer", server_url + "/event", {}, null);
setTimeout(() => {  // We put a slight delay in here, in order fgor the app information callback to propagate before updating the number
    nexmo.number.update(region, vfrom, { "voiceCallbackType": "app", "voiceCallbackValue": app_id }, null);
}, 1000);
...
app.post('/connect', async (req, res) => {
    let phone = req.body.phone;  // THe phone num,ber of the customer, passed in from the GUI
    nexmo.calls.create({
        to: [{
            type: 'phone',
            number: phone
...
app.get('/answer', (req, res) => {
    ...
    */
    nexmo[req.query.uid].calls.create({
        to: [{
            type: 'websocket',
...
app.get('/lex_answer', (req, res) => {
    ...
    nexmo[uid].calls.talk.start(lex_uuid,
        {
            text: phrase, voice_name: voice,
            loop: 1
        }, (err, res) => {
            if (err) { console.log('>>> TTS Talk to User (2) ' + lex_uuid + ' error'); }
        });
    let nccoResponse = [
        {
            "action": "conversation",
...
function transfer(uid) {
    ...
    nexmo.calls.talk.start(ncustomer_call_uuid, { text: phrase, voice_name: voice, loop: 1 }, (err, res) => {
        if (err) { console.log('>>> TTS Talk to User (2) ' + customer_call_uuid + 'error'); }
    });
    // Hangup lex, if still there:
    if (lex_uuid && lex_uuid.length) {
        console.log("Hanging up Lex side...");
        nexmo.calls.update(lex_uuid, { action: 'hangup' }, (err, res) => {
            lex_uuid = null;
        });
    }
}
...
function connectSockets(uid) {
    ...
    nexmo.calls.create({
        ...
        if (err) {
            console.error(">>> Websocket 2 create error:", err);
        }
        else {
            console.log(">>> Customer websocket 2 create status:", res);
        }
    });
    ...
    nexmo.calls.create({
        ...
        if (err) {
            console.error(">>> Websocket Agent 2 create error:", err);
        }
        else {
            console.log(">>> Agent websocket 2 create status:", res);
        }
    });
}
